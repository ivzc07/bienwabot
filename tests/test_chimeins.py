...
Three separate rules live in one object here, and the tests keep them separate:
the probability that makes her selective, the daily ceiling that is absolute
whatever the probability says, and the cooldown that stops two landing together.
...
from rebe_agent.chimeins import (
    CHANCE,
    CHIME_INS_PER_DAY,
    COOLDOWN,
    ChimeInBudget,
...
async def test_most_eligible_messages_get_nothing() -> None:
    ...
    clock = ManualClock(NOON)
    allowed = budget(clock, rng=random.Random(SEED), per_day=10_000, cooldown=timedelta(0))
...
async def test_the_daily_ceiling_holds_whatever_the_roll_says() -> None:
    ...
    day = budget(clock, log, rng=Rolls(*[0.0] * (CHIME_INS_PER_DAY + 1)), cooldown=timedelta(0))

    for _ in range(CHIME_INS_PER_DAY):
        assert await day.refuses() is None
        await day.spend(GROUP)

    assert await day.refuses() == f"{CHIME_INS_PER_DAY} unprompted chime-ins today already"
...
async def test_two_chime_ins_never_land_back_to_back() -> None:
    ...
    assert refusal is not None and "too soon" in refusal
...
async def test_the_day_turns_over_at_local_midnight_not_at_utc_midnight() -> None:
    ...
    log = InMemoryChimeInLog()
    day = budget(clock, log, rng=Rolls(*[0.0] * 10), cooldown=timedelta(0))
...
async def test_a_fresh_local_day_is_a_fresh_allowance() -> None:
    ...
    assert await day.refuses() is None, "01:00 the next morning is a new day"
...
async def test_the_count_outlives_the_budget_that_wrote_it() -> None:
    ...
    clock = ManualClock(NOON)
    log = InMemoryChimeInLog()
    before = budget(clock, log, rng=Rolls(*[0.0] * 10), cooldown=timedelta(0))
    for _ in range(CHIME_INS_PER_DAY):
        await before.spend(GROUP)

    after_restart = budget(clock, log, rng=Rolls(*[0.0] * 10), cooldown=timedelta(0))
